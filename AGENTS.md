...
- server/                FastAPI app and indexing pipeline
  - app/
    - api/               Routers for health, documents, query, wiki
    - core/              Settings, dependencies
    - db/                SQLModel models, session
    - rag/               Chunking, embeddings, vector store
    - services/          Parsing, classification, ingestion, wiki builder
    - tests/             Backend unit tests
  - pyproject.toml       Python project config
- web/                   Next.js app (App Router)
  - app/                 Pages: /, /documents, /chat, /wiki
...
- Python: 3.11+
- FastAPI: ~=0.121.1 (Pydantic v2)
- SQLModel: >=0.0.27 (Pydantic v2)
- Pydantic: v2 series
...
- OFFLINE_MODE: set true to use deterministic embeddings and keyword labels (no network)
...
- Embedding: OpenAI text-embedding-3-small (1536 dims). Store vectors + metadata in LanceDB. Offline fallback uses SHA256-based deterministic vectors (1536-dim, unit-normalized).
- Classification: LLM suggests up to 3–5 labels per doc; offline fallback uses keyword frequency.
- Query: Vector retrieval with optional label filters. Return answer and citations (document titles and chunk indices). Offline fallback returns extractive summary from retrieved chunks.
- Wiki: For each label, gather top chunks across docs; synthesize a markdown article: lead summary, sections, and references strictly citing the user’s content. Updates when documents change or on rebuild.
...
- POST /query: {query, labels?, k?} -> {answer, citations}
- GET /wiki: list available labels/categories
- GET /wiki/{label}: fetch rendered article (markdown)
- POST /wiki/rebuild: rebuild articles for all labels

Local Dev Commands
- Backend:
  - cd server && python -m venv .venv && source .venv/bin/activate && pip install -e .[test]
...
- Do NOT commit secrets. Provide .env.example and read from .env / .env.local.
...
- Provide unit tests for the RAG pipeline and API endpoints. Prefer offline mode in tests.
